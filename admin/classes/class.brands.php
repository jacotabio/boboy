...
class Brands{
  ...
  public function get_brands(){
  	$sth = $this->db->prepare("SELECT brands.brand_id,brand_name,usr_email,usr_address,usr_contact,CASE WHEN brand_status = 1 THEN 'Online' WHEN brand_status = 0 THEN 'Offline' END AS status FROM brands,users WHERE brands.brand_id = users.brand_id AND is_hidden = 0");
  	$sth->execute();
  ...
  public function update_brand($id,$name,$email,$phone,$address){
    $sth = $this->db->prepare("UPDATE users SET usr_name = ?, usr_email = ?,usr_contact = ?, usr_address = ?, usr_status = ? WHERE brand_id = ?");
