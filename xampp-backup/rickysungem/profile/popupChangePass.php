<?php
if($user->get_session()){
?><div id="popup-modal-2">
<div id="products-popup-order">
	<div class="modal-header">
		<span onclick ="div_hide()" class="close">&times;</span>
		<div id="popup-profile-title">Change Password</div>
	</div>
	<div class="modal-body">
	<?php
		$id = $_SESSION['c_userid'];
		$list = $user->get_profile_info($id);
		foreach($list as $value){
	?>
<form method="POST" id="change_pass_form" class="font-roboto">
<input type="hidden" name="indicator-pass" value="1">
<input class="popup_profile" type="hidden" id="usr_id" name="usr_id" value="<?php echo $value['client_id']?>" required/>
<input class="popup_profile" type="hidden" id="pass" name="pass" value="<?php echo $value['usr_password']?>" required/>
<div class="popup-edit-title">Current Password <span class="redcol"> <strong>*</strong></span></div>	
<input class="popup_profile" type="password" id="current-pass" name="current-pass" required/>
<div class="popup-edit-title">New Password<span class="redcol"> <strong>*</strong></span></div>	
<input class="popup_profile" type="password" id="new-pass" name="new-pass" required/>
<div class="popup-edit-title">Confirm New Password<span class="redcol"> <strong>*</strong></span></div>	
<input class="popup_profile" type="password" id="confirm-new-pass" name="confirm-new-pass" required/>
	<?php
		
	}
	?>
	</div>
<div class="modal-footer">
	<input type="button" onclick="passDoesNotMatch();" id="submit-change-pass" class="popup-btn" value="Change Password">
		<!--<button type="submit" class="popup-qty-btn" onclick="confirm_cart()" name="submit">Save edit</button>-->
</div>
</form>
</div>
</div>
<?php
}
?>
